...
function CreateNestedGrid(Model, elem) {
    ...
    var newGrid = $(elem).parents('tr:first').next("tr.SubGrid").find("table.Gridtable:last");
    CheckautoOpenGrids(newGrid);
    Helight_Search(newGrid, Model.SearchData);
        ...
        if (col.Visible) {
            ...
            if (col.Sortable || col.Searchable) {
                if (col.Sortable) {
                    ...
                    res += "<span Prop-Name ='" + col.Property + "' class='" + col.CssClass + "'>" + col.DisplayName + "</span>";
                }
...
function ShowBusy(Grid, res) {
    var GridContainer = $(Grid).parents("div.gridcontner");
    var LoaderImg = $(GridContainer).find("div.loader-img")[0];
    ...
    if (res) {
        ...
        if (LoaderImg == undefined)
            LoaderImg = $("div.loader-img:visible");
        $(LoaderImg).hide();
        //$('.div_busy_panel_background').hide();
        $(GridContainer).css('position', 'inherit');
    }
};
